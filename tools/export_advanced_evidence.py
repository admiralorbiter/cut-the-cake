...
def main():
    ...
    # Also emit presentations_data.js for zero-server / double-click file:/// execution
    output_js = os.path.join(REPO_ROOT, "explainer", "advanced", "presentations_data.js")
    with open(output_js, "w", encoding="utf-8") as f:
        f.write("/** Auto-generated by tools/export_advanced_evidence.py — Zero-server standalone data bundle **/\n")
        f.write("window.PRESENTATIONS_DATA = ")
        json.dump(payload, f, indent=2)
        f.write(";\n")

    file_size_kb = os.path.getsize(OUTPUT_JSON) / 1024.0
    print(f"[OK] Wrote 8 authoritative presentations to {OUTPUT_JSON} and {output_js} ({file_size_kb:.1f} KB)")
