...
# =============================================================================
# GATE 6C-1: PLANAR CONTROLLER BIT-FOR-BIT TELEMETRY PARITY
# =============================================================================

def test_m6c_gate1_planar_controller_bit_for_bit_parity():
    """Gate 6C-1: Verify that 3D-capable controller reproduces identical telemetry and completions on all planar fixtures."""
    fixtures = [
...
# =============================================================================
# GATE 6C-2: PURE ELEVATION SLEW EXECUTION PARITY
# =============================================================================

def test_m6c_gate2_pure_elevation_slew_execution_parity():
    """Gate 6C-2: Verify pure vertical pitch slew at omega = 360 deg/s matches scheduled tics exactly."""
    ...
    # 30 deg / 10.2857 deg/tic = 2.916 -> ceil = 3 slew tics
    # Acquisition = 3 tics
    # Service = 3 tics
    # Expected service complete tic = 3 (slew) + 3 (acq) + 3 (svc) - 1 = 8 (scheduled_end_tic)
    ...
    assert job["realized_service_complete_tic"] == 8
...
# =============================================================================
# GATE 6C-3: MIXED (THETA, PHI) SPHERICAL ARC SLEW EXECUTION
# =============================================================================

def test_m6c_gate3_mixed_spherical_arc_slew_execution():
    """Gate 6C-3: Verify mixed azimuth-elevation slew follows great-circle spherical geodesic."""
    params = TicCombatParameters(
        aim_velocity_deg_s=360.0,         # 10.2857 deg/tic
    ...
    # Transition from (-45 deg, 60 deg) to (+45 deg, 60 deg)
    # alpha = arccos(sin(60)^2 + cos(60)^2 * cos(90)) = arccos(0.75 + 0) = arccos(0.75) ~ 41.41 deg
    # Slew tics = ceil(41.41 / 10.2857) = 5 tics
    ...
    for step in range(5):
        ...
        # Verify that pitch stays high in spherical arc
        assert next_ph > 50.0
        curr_th, curr_ph = next_th, next_ph

    # Must arrive at target exactly
    assert math.isclose(curr_th, 45.0, abs_tol=1e-4)
    assert math.isclose(curr_ph, 60.0, abs_tol=1e-4)


# =============================================================================
# GATE 6C-4: 3D RAMP DYNAMIC TRAVERSAL & SLEW TRACKING
# =============================================================================

def test_m6c_gate4_3d_ramp_dynamic_traversal_and_slew_tracking():
    """Gate 6C-4: Verify player traversing 3D ramp emits 3D position and dynamic reticle pitch in telemetry."""
    doc = CADDocument(
    ...
    # Start: player at (0, 0, 0), eye at 1.65m, target at (25, 0, 6.65m) -> dz=5.0, dxy=25 -> phi = 11.31 deg
    first_frame = frames[0]
    assert first_frame["player_pos"] == [0.0, 0.0, 0.0]

    # End of ramp: player at (20, 0, 5.0), eye at 6.65m, target at (25, 0, 6.65m) -> dz=0.0, dxy=5 -> phi = 0.0 deg
    last_frame = frames[-1]
...
def test_m6c_gate5_realized_service_completion_vs_schedule_identity():
    """Gate 6C-5: Verify realized service complete tic matches discrete schedule completion tic on 3D fixtures."""
    doc = get_ascent_a_main_document()
    doc.player_model.elevation_mode = ElevationMode.GEOMETRIC
    heaven = next(t for t in doc.threats if t.id == "threat_site_deep")
    heaven.z_m = 4.65

    res = analyze_cad_document(doc, route_id="route_A", include_telemetry=True)
    assert res["is_valid"] is True
    assert res["telemetry_status"] == "SUCCESS"

    # In Ascent A-Main with Heaven, all revealed threats are successfully serviced
    for j in res["threat_jobs"]:
        if res["model_episode_survived"]:
            assert j["realized_service_complete_tic"] == j["scheduled_service_end_tic"]
...
def test_m6c_gate6_3d_post_death_timeout_invariant():
    """Gate 6C-6: Verify controller records death and freezes physical position upon 3D deadline breach."""
    doc = CADDocument(
